...
//  Three Letter Word
//
//  Created by Samuel Amante on 11/12/24.
//
    ...
    var body: some View {
        VStack {
            ...
            CustomLetterBox(color: .gray, text: letter)
                .onTapGesture {
                    let position = alphabet.index(alphabet.startIndex, offsetBy: counter);letter = String(alphabet[position])
                    counter += 1
                    if counter == alphabet.count {
                        counter = 0
                    }
                }
            Text("Tap a red box to select the letter")
            HStack {
